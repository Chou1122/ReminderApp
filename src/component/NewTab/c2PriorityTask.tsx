...
export default function C2PriorityTaskTab() {
  ...
  const c2PriorityTaskOpen = useSelector(
    (state: RootState) => state.c2PriorityTaskOpenRedux.openTab
  );

  const navigation: any = useNavigation();

  const backOnPress = () => {
    dispatch(actionSetC2PriorityTaskOpen(false));
  };
              ...
              <View className="h-[240] w-full pl-5 pr-5">
                <View className="h-full w-full bg-white rounded-2xl">
                  <View className="flex-row justify-between h-[25%] w-full border-b-4 border-gray-200">
                    <TouchableOpacity
                      className="h-full w-[40%] justify-center pl-5"
                      onPress={() => {
                        noneOnPress();
                      }}
                    >
                      <Text className="text-black text-base text-medium">
                        None
                      </Text>
                    </TouchableOpacity>
                    {priorityState == "None" && (
                  ...
                  <View className="flex-row justify-between h-[25%] w-full pl-5">
                    <View className="flex-row justify-between h-full w-full border-b-2 border-gray-200">
                      <TouchableOpacity
                        className="h-full w-[40%] justify-center"
                        onPress={() => {
                          lowOnPress();
                        }}
                      >
